...
bool compare(int a[], int b[], int type){
    ...
    //type 2 兩對
        ...
        if(a_same > b_same) return true;
            ...
            for(int i=0;i<5;i++){
                if(a[i]%10 > color_a && a[i]/10==a_same) color_a=a[i]%10;
                if(b[i]%10 > color_b && b[i]/10==b_same) color_b=b[i]%10;
            }
            if(color_a>color_b) return true;
            else return false;
        }
    }
    //type 3 三條
    ...
    /*
        ...
        int com_a, com_b;
        if(a[0]/10==2 || b[0]/10==2){
            if(a[0]/10==2 && b[0]/10==2){
                if (a[0]%10>b[0]%10) return true;
                else return false;
            }
            else if(a[0]/10==2) com_a=11;
            else if(b[0]/10==2) com_b==11;
        }
    }*/
    //type 5 同花
    else if(type == 5){
