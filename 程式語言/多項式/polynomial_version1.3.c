...
node *muti(node *first_x, node *first_y, node *first_z){
    //創好link listed
    int max_exp = first_x->exp + first_y->exp;
    node *now = first_z;
    ...
    int now_exp = max_exp;
    int max_x_exp = first_x->exp;
    int max_y_exp = first_y->exp;
    node *index_x, *index_y, *index_z;;
    while(now_exp>=0){
        //printf("now_exp = %d\n", now_exp);
        index_z = find_exp(first_z ,now_exp);
        for(int i=max_x_exp;i>=0;i--){
            for(int j=max_y_exp;j>=0;j--){
                if((i+j)!=now_exp) continue;
                if((i+j)<now_exp) break;
                //printf("i=%d, j=%d\n", i, j);
                index_x = find_exp(first_x, i);
                index_y = find_exp(first_y, j);
                if(index_x!=NULL && index_y!=NULL){
                    //printf("index_x->num = %d\n", index_x->num);
                    index_z->num += ((index_x->num) * (index_y->num));
                }
            }
            //printf("Test\n\n");
        }
        ...
        /*
        for(int i=first_x->exp;i>=0;i--){
            for(int j=first_y->exp;j>=0;j--){
                if((i+j) == now_exp)
                index_x = find_exp(first_x, i);
                index_y = find_exp(first_y, j);
                if(index_x!=NULL && index_y!=NULL){
...
int main(){
    ...
    first_z = muti(first_x, first_y, first_z);
    print_answer(first_z);
    Free(first_z);
